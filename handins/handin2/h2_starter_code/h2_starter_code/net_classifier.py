...
def get_init_params(input_dim, hidden_size, output_size):
    ...
    W1 = np.random.normal(0, np.sqrt(
        2./(input_dim+hidden_size)), size=(input_dim, hidden_size))
    b1 = np.zeros((1, hidden_size))
    W2 = np.random.normal(0, np.sqrt(
        4./(hidden_size+output_size)), size=(hidden_size, output_size))
...
class NetClassifier():
    ...
    def predict(self, X, params=None):
        ...
        pred = None
        # YOUR CODE HERE
        pred = np.argmax(self.forward(X, params), axis=1)
        # END CODE
        return pred
    ...
    def cost_grad(X, y, params, c=0.0):
        ...
        cost = -np.sum(labels * np.log(y_hat)) / \
            X.shape[0] + c * (np.sum(W1 * W1) + np.sum(W2 * W2))
        print(cost)
        # END CODE

        # YOUR CODE HERE - BACKWARDS PASS - compute derivatives of all weights and bias, store them in d_w1, d_w2, d_b1, d_b2
        d_z = y_hat - labels
        d_w2 = np.dot(c1.T, d_z) / X.shape[0] + 2 * c * W2
        d_b2 = np.sum(d_z, axis=0, keepdims=True) / X.shape[0]
        d_h = np.dot(d_z, W2.T)
        d_a = d_h * (b > 0)
        d_w1 = np.dot(X.T, d_a) / X.shape[0] + 2 * c * W1
        d_b1 = np.sum(d_a, axis=0, keepdims=True) / X.shape[0]
        # END CODE
...
def numerical_grad_check(f, x, key):
    ...
    while not it.finished:
        ...
        assert np.abs(
            num_grad - grad[dim]) < eps, 'numerical gradient error index {0}, numerical gradient {1}, computed gradient {2}'.format(dim, num_grad, grad[dim])
        it.iternext()
...
def test_grad():
    ...
    def f(z): return nc.cost_grad(X, y, params, c=1.0)
